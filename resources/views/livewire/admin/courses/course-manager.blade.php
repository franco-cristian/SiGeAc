<div>
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table class="hidden md:table w-full text-left table-auto">
                            <thead class="bg-gray-50 dark:bg-gray-700/50">
                                <tr>
                                    ...
                                    <th class="w-1/5 px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Horarios</th>
                            ...
                            <tbody class="bg-white dark:bg-dark-neutral-card divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                                    <td class="px-4 py-4 align-top">
                                        <p class="font-medium text-secondary dark:text-dark-secondary">{{ $course->subject->name }}</p>
                                    ...
                                    <td class="px-4 py-4 align-top text-sm text-gray-500 dark:text-gray-400">
                                        <ul class="space-y-1">
                                            ...
                                            <li class="capitalize whitespace-nowrap">{{ $schedule->day_of_week }}: {{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}</li>
                                            @endforeach
                                        </ul>
                                    </td>
                                    ...
                                    <td class="px-4 py-4 align-top text-center text-sm font-medium space-x-2">
                                        <button wire:click="editCourse({{ $course->id }})" class="p-1 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200"><x-lucide-file-pen-line class="h-5 w-5" /></button>
                                        <button wire:click="confirmCourseDeletion({{ $course->id }})" class="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200"><x-lucide-trash-2 class="h-5 w-5" /></button>
                                    </td>
                                </tr>
                                @empty
                                <tr>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg shadow space-y-3">
                                <div>
                                    <p class="font-bold text-secondary dark:text-dark-secondary">{{ $course->subject->name }}</p>
                                ...
                                <div class="text-sm text-gray-500 dark:text-gray-400"><strong>Horarios:</strong>
                                    <ul class="list-disc list-inside ml-2">
                                        @foreach($course->schedules as $schedule)
                                        <li class="capitalize">{{ $schedule->day_of_week }}: {{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                                <div class="flex justify-between items-center text-sm pt-3 border-t border-gray-200 dark:border-gray-600">
                                    <span class="text-gray-500 dark:text-gray-400"><strong>Cupo:</strong> {{ $course->capacity }}</span>
                                    <div class="space-x-2">
                                        <button wire:click="editCourse({{ $course->id }})" class="p-1 text-gray-400 hover:text-blue-600 ..."><x-lucide-file-pen-line class="h-5 w-5" /></button>
                                        <button wire:click="confirmCourseDeletion({{ $course->id }})" class="p-1 text-gray-400 hover:text-red-600 ..."><x-lucide-trash-2 class="h-5 w-5" /></button>
                                    </div>
                                </div>
                            </div>
                            @empty
    ...
    <div class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <h3 class="text-xl font-bold text-secondary dark:text-dark-secondary px-6 pt-6">
                {{ $editingCourse ? 'Editar Curso' : 'Crear Nuevo Curso' }}
            </h3>

            <form wire:submit="saveCourse" class="flex flex-col flex-grow">
                <div class="mt-6 space-y-4 flex-grow overflow-y-auto px-6">
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                        <div class="flex justify-between items-center">
                            ...
                            <x-secondary-button type="button" wire:click="addSchedule">
                                <x-lucide-plus class="w-4 h-4" />
                            </x-secondary-button>
                        </div>
                        <div class="mt-4 space-y-4">
                            @forelse($schedules as $index => $schedule)
                            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-center" wire:key="schedule-{{ $index }}">
                                <div>
                                ...
                                <div class="pt-5 text-center">
                                    <x-danger-button type="button" wire:click="removeSchedule({{ $index }})">
                                        <x-lucide-trash-2 class="w-4 h-4" />
                                    </x-danger-button>
                                </div>
                            </div>
                            <div class="flex flex-wrap">
                                <x-input-error :messages="$errors->get('schedules.'.$index.'.day_of_week')" class="mt-1 text-xs" />
                                <x-input-error :messages="$errors->get('schedules.'.$index.'.start_time')" class="mt-1 text-xs" />
                                <x-input-error :messages="$errors->get('schedules.'.$index.'.end_time')" class="mt-1 text-xs" />
                            </div>
                            @empty
                ...
                <div class="mt-auto flex justify-end space-x-4 border-t border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
