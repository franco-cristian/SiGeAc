<div>
    ...
    <div class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg">
            <h3 class="text-xl font-bold text-secondary dark:text-dark-secondary">Editar Usuario</h3>
            <p class="text-sm text-neutral-text dark:text-dark-neutral-text">Modificando el perfil de: <span class="font-semibold">{{ $userToEdit->name }}</span></p>

            <form wire:submit="save" class="mt-6 space-y-4">
                <div>
                    <label for="editingName" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre</label>
                    <x-text-input wire:model="editingState.name" id="editingName" class="mt-1 w-full" />
                    <x-input-error :messages="$errors->get('editingState.name')" class="mt-1" />
                </div>
                <div>
                    <label for="editingEmail" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                    <x-text-input wire:model="editingState.email" id="editingEmail" type="email" class="mt-1 w-full" />
                    <x-input-error :messages="$errors->get('editingState.email')" class="mt-1" />
                </div>
                <div>
                    <label for="editingDni" class="block text-sm font-medium text-gray-700 dark:text-gray-300">DNI</label>
                    <x-text-input wire:model="editingState.dni" id="editingDni" class="mt-1 w-full" />
                    <x-input-error :messages="$errors->get('editingState.dni')" class="mt-1" />
                </div>
                <div>
